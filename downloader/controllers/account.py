            ...
            if valid_count is None:
                ...
                if valid_count == 0:
                    ...
                    if csdn_account.need_sms_validate:
                        ...
                        feishu_send_message(text=msg, user_id=settings.FEISHU_USER_ID)
                ding(
                    f"[CSDN] 剩余下载个数：{valid_count}", download_account_id=csdn_account.id
                )
        ...
        for baidu_account in baidu_accounts:
            ...
            with requests.post(get_user_info_url, headers=headers) as r:
                if r.status_code == requests.codes.OK:
                    ...
                    if is_login:
                        share_doc_count = data["jiaoyu_vip_info"][
                            "download_ticket_count"
                        ]
                        vip_special_doc_count = data["jiaoyu_vip_info"][
                            "professional_download_ticket_count"
                        ]
                        ding(
                            f"[百度文库] 可用共享文档下载特权 {share_doc_count} 次，可用VIP专享文档下载特权 {vip_special_doc_count} 次",
...
def check_docer_cookies(request):
    ...
    if token == settings.ADMIN_TOKEN:
        try:
            ...
            headers = {"cookie": docer_account.cookies}
            url = "https://www.docer.com/proxy-docer/v4.php/api/user/allinfo"
            with requests.get(url, headers=headers) as r:
                if r.json()["result"] == "ok":
                    ding("稻壳模板cookies仍有效", download_account_id=docer_account.id)
                else:
                    ding(
                        "稻壳模板cookies已失效，请尽快更新",
                        download_account_id=docer_account.id,
                        need_email=True,
                    )
        except DocerAccount.DoesNotExist:
            ding("没有可以使用的稻壳模板会员账号", need_email=True)
...
def reset_csdn_today_download_count(request):
    ...
    if token == settings.ADMIN_TOKEN:
        ...
        for csdn_account in csdn_accounts:
            ...
            csdn_account.save()
            ding("[CSDN] 今日下载数已重置", download_account_id=csdn_account.id)
        ...
        with requests.get("https://dl.58pic.com/35979263.html", headers=headers) as r:
            if r.status_code == requests.codes.OK:
                ...
                if len(soup.select("a.clickRecord.autodown")) > 0:
                    ding("[千图网] Cookies仍有效", download_account_id=qiantu_account.id)
                else:
                    ding(
                        "[千图网] Cookies已失效, 请尽快更新！",
...
def add_or_update_csdn_account(request):
    ...
    if valid_count == 0:
        return JsonResponse(
            dict(code=requests.codes.bad_request, msg="CSDN会员账号剩余可用下载数为零！")
        )
    ...
    if csdn_account_id:
        try:
            ...
            csdn_account.save()
            return JsonResponse(
                dict(code=requests.codes.ok, msg="成功更新CSDN会员账号的Cookies")
            )
        ...
        if CsdnAccount.objects.filter(csdn_id=csdn_id).count() > 0:
            return JsonResponse(
                dict(code=requests.codes.bad_request, msg="CSDN账号已存在，请勿重复添加！")
            )

        CsdnAccount(
            user=user, cookies=cookies, valid_count=valid_count, csdn_id=csdn_id
        ).save()
...
def list_csdn_accounts(request):
    ...
    return JsonResponse(
        dict(
            ...
            csdn_accounts=CsdnAccountSerializers(csdn_accounts, many=True).data,
        )
    )
