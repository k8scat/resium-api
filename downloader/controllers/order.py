...
def alipay_notify(request):
    ...
    if success and data["trade_status"] in ("TRADE_SUCCESS", "TRADE_FINISHED"):
        ...
        try:
            ...
            ding(f"收入+{total_amount}元", uid=order.user.uid)
...
def mp_pay_notify(request):
    ...
    try:
        ...
        ding("[微信支付] 签名校验失败", error=request.body)
        return_code = "FAIL"
        return_msg = "签名失败"
    ...
    with requests.get(
        ...
        if r.status_code == requests.codes.OK:
            ...
            if data.get("errcode", 0) == 0:  # 没有errcode或者errcode为0时表示请求成功
                ...
                if (
                    ...
                    return JsonResponse(
                        dict(
                            ...
                            params=we_chat_pay.jsapi.get_jsapi_params(
                                prepay_id=prepay_id
                            ),
                        )
                    )
                ...
                ding(
                    "[微信支付] 创建订单失败", error=json.dumps(create_order_res), need_email=True
                )
                return JsonResponse(
            ...
            ding(f"[微信支付] auth.code2Session接口调用失败", error=r.text, need_email=True)
...
def create_order(request):
    ...
    try:
        ...
        return JsonResponse(
            dict(code=requests.codes.ok, order=OrderSerializers(order).data)
        )
