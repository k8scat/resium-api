...
def ding(
    ...
    if need_email:
        send_email(
            subject="[源自下载] 服务状态告警", content=content, to_addr=settings.ADMIN_EMAIL
        )
...
def aliyun_oss_upload(filepath: str, key: str):
    """
    阿里云 OSS 上传
...
def check_oss(resource_url):
    ...
    try:
        ...
        if not aliyun_oss_check_file(resource.key):
            resource.delete()
            ding(f"OSS资源未找到，请及时检查资源 {resource.key}", need_email=True)
            return None
        return resource
...
def check_download(folder):
    ...
    for i in range(3000):
        ...
        ding(f"下载超时: {folder}", logger=logging.error, need_email=True)
        return 500, "检查下载文件超时"
...
def save_resource(
    ...
    try:
        ...
        aliyun_oss_upload(filepath, key)
...
def predict_code(image_path):
    ...
    files = {"img_data": ("img_data", open(image_path, "rb").read())}
    ...
    with requests.post(url, data, files=files, headers=headers) as r:
        if r.status_code == requests.codes.OK:
            ...
            if result["RetCode"] == "0":
                ...
                key = (
                    get_unique_str() + "." + os.path.basename(image_path).split(".")[-1]
                )
                if qiniu_upload(settings.QINIU_OPEN_BUCKET, image_path, key):
                    image_url = qiniu_get_url(key)
                    ding(f"验证码识别成功: {code}", image=image_url)
                else:
                    ding(f"验证码识别成功: {code}", error="七牛云上传图片失败", need_email=True)
                return code
            ding(f"验证码识别失败: {r.content.decode()}", need_email=True)
            return None
        ding(f"验证码识别请求失败: {r.status_code} {r.content.decode()}", need_email=True)
        return None
        ...
        with requests.post(
            ...
            if r.status_code == requests.codes.OK:
                ...
                if resp["code"] == 200:
                    ...
                    # 上传失败
                    ding(
                        "资源上传到CSDN失败", error=resp, logger=logging.error, need_email=True
                    )
...
def get_short_url(url, long_term=False):
    ...
    with requests.post(
        "https://dwz.cn/admin/v2/create", data=json.dumps(body), headers=headers
    ) as r:
        if r.status_code == requests.codes.OK and r.json()["Code"] == 0:
            return r.json()["ShortUrl"]
        else:
            ding("[短网址] 生成失败", error=r.text, need_email=True)
            return None
...
def get_long_url(url):
    ...
    body = {"shortUrl": url}
    with requests.post(
        "https://dwz.cn/admin/v2/query", data=json.dumps(body), headers=headers
    ) as r:
        if r.status_code == requests.codes.OK and r.json()["Code"] == 0:
...
def switch_csdn_account(csdn_account, need_sms_validate=False):
    ...
    valid_admin_csdn_accounts = DownloadAccount.objects.filter(
        status=STATUS_DISABLED,
        today_download_count__lt=20,
        need_sms_validate=False,
        is_disabled=False,
        csdn_id__in=settings.ADMIN_CSDN_ACCOUNTS,
    ).all()

    valid_csdn_accounts = CsdnAccount.objects.filter(
        is_enabled=False,
        today_download_count__lt=20,
        need_sms_validate=False,
        is_disabled=False,
    ...
    if len(valid_csdn_accounts) > 0:
        ...
        if not need_sms_validate:
            ...
            if valid_count is None:
                ...
                ding("[CSDN] Cookies已失效", download_account_id=csdn_account.id)
            elif valid_count == 0:
                csdn_account.is_disabled = True
            ...
            send_email(
                subject="[源自下载] CSDN账号提醒", content=msg, to_addr=csdn_account.user.email
            )
            feishu_send_message(text=msg, user_id=settings.FEISHU_USER_ID)

        csdn_account.save()
        ding("[CSDN] 自动切换账号成功", download_account_id=csdn_account.id)
        return new_csdn_account
    else:
        csdn_account.need_sms_validate = need_sms_validate
        csdn_account.save()
        ding(
            "[CSDN] 自动切换账号成功失败，没有可用的CSDN账号",
    ...
    def decrypt(self, encrypted_data, iv):
        ...
        try:
            ...
            if app_id != self.app_id:
                ding(f"[小程序登录] wrong appid: {app_id}", need_email=True)
                return None

            return decrypted
        except KeyError:
            ding("[小程序登录] decrypt KeyError", need_email=True)
            return None
    ...
    while True:
        if User.objects.filter(uid=uid).count():
            ...
            if repetition_count > 0:
                ding(f"UID生成重复次数: {repetition_count}", uid=uid, need_email=True)
            return uid
...
def get_csdn_valid_count(cookies):
    ...
    with requests.get(
        "https://download.csdn.net/api/source/index/v1/loginInfo", headers=headers
    ) as r:
        if r.status_code != requests.codes.ok:
...
def get_csdn_id(cookies):
    ...
    try:
        headers = {"cookie": cookies, "user-agent": get_random_ua()}
        ...
        with requests.post(url, json=payload) as r:
            if r.status_code == requests.codes.ok:
                ...
                if data.get("code", -1) == 0:
                    ...
                    if token:
                        cache.set(
                            "feishu_token", token, timeout=settings.DOWNLOAD_INTERVAL
                        )
                        return token
                    else:
                        ding(
                            message="[飞书] access_token获取失败",
                            error=data["msg"],
                            logger=logging.error,
                            need_email=True,
                        )
                ...
                ding(
                    message=f"[飞书] 接口请求失败, code={r.status_code}, content={str(r.content)}",
                    logger=logging.error,
                    need_email=True,
                )
                return None
...
def feishu_send_message(
    text, chat_id=None, open_id=None, user_id=None, email=None, root_id=None
):
